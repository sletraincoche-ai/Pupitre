...
// Point d'entrée du flux de connexion Gmail (gmail.send uniquement),
// depuis Studio IA. Le `state` transporte à la fois la page de retour et
// un jeton anti-CSRF léger vérifié par le callback.
export async function GET(request: NextRequest) {
  ...
  if (!isGoogleConfigured()) {
    return NextResponse.redirect(`${origin}${retour}?google_error=not_configured`);
  }
