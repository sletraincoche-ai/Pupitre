...
export function EditPanel({
  ...
  return (
    <div className="flex flex-col gap-3">
      <div>
        <p className="mb-1.5 text-xs font-medium tracking-wide text-white/60 uppercase">Photos</p>
        <PhotoPicker selection={edited.photos} onChange={(photos) => onChange({ ...edited, photos })} />
      </div>

      <div>
        <label className="mb-1.5 block text-xs font-medium tracking-wide text-white/60 uppercase">
          Légende
        </label>
        <textarea
          ...
          className="w-full rounded-xl border border-white/20 bg-white/10 px-3 py-1.5 text-sm text-white outline-none placeholder:text-white/40 focus-visible:border-white/40 focus-visible:ring-3 focus-visible:ring-white/20"
        />
      </div>
